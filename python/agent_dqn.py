...
class ReplayMemory:
    ...
    def populate(self, env, num_steps):
        ...
        env.reset()
        state = env.Preprocess(env.observations())
        ...
        for i in range(num_steps):
            ...
            if done:
                env.reset()
            next_state = env.Preprocess(env.observations())
...
class DQN(nn.Module):
    def __init__(self, resolution, action_dim, *, num_layers=4, hidden_dim=256):
        ...
        #     nn.Conv2d(resolution[0], 96, kernel_size=11, stride=4, padding=0),
        #     nn.BatchNorm2d(96),
        #     nn.ReLU(),
        ...
        #     nn.Linear(4096, self.action_dim))
...
if __name__ == '__main__':
  ...
  # `config` key/value dictionary.
    ...
    if args.Test:
        returns = Test(args.model, num_steps, False)
        
        plt.figure(2)
        plt.xlabel('Episodes')
        plt.ylabel('Return')
        plt.title('DQN LeNet5')
