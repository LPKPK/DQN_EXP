...
class DQN(nn.Module):
    def __init__(self, resolution, action_dim, *, num_layers=4, hidden_dim=256):
        ...
        self.lstm = nn.LSTM(49, LSTM_MEMORY, 1)

        self.fc1 = nn.Linear(32 * LSTM_MEMORY, self.hidden_dim)  # For not 40 * 40, Adjust the input size based on your resolution
    ...
    def forward(self, x, hidden_state, cell_state) -> torch.Tensor:
        ...
        return q_values, next_hidden_state, next_cell_state
...
def train_dqn(
    ...
    *,
    ...
    dqn_hidden_state, dqn_cell_state = dqn_model.init_states()
    target_hidden_state, target_cell_state = dqn_target.init_states()
...
def Test(model_name, num_steps):
    try:
        checkpoint = torch.load(model_name)
    except FileNotFoundError:
        pass
    else:
        value = checkpoint['80_0']
...
if __name__ == '__main__':
  ...
  # `config` key/value dictionary.
    ...
    replay_size = 100_000
    replay_prepopulate_steps = 5_000  # 50_000

    batch_size = 64
    exploration = ExponentialSchedule(1.0, 0.01, num_steps * 0.8)

    if args.Test:
        Test(args.model, num_steps)
    else:
